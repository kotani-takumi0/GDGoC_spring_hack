...
async function callGenerateCodeApi(
  ...
  const response = await fetch("/api/generate-code", {
    ...
    body: JSON.stringify({ scenarioId, experienceLevel, useLocalLLM }),
  });
...
function Step3Content() {
  ...
  const generateCode = useCallback(async () => {
    ...
    if (demoData) {
      ...
      router.push(`/step/4?scenario=${scenarioId}&level=${level}&mode=${mode}&useLocalLLM=${useLocalLLM}`);
      return;
    }
    ...
    try {
      const result = await callGenerateCodeApi(scenarioId, level, useLocalLLM);
  ...
  }, [scenarioId, level, mode, useLocalLLM, router]);
