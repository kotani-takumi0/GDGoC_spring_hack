...
function Step2Content() {
  ...
  useEffect(() => {
    ...
    async function fetchPersonalized() {
      ...
      try {
        const responsePromise = fetch("/api/personalize", {
          ...
          body: JSON.stringify({ scenarioId, experienceLevel: level, useLocalLLM }),
        });
    ...
    fetchPersonalized();
  }, [scenarioId, level, scenario, useLocalLLM]);
  ...
  const handleNext = useCallback(() => {
    router.push(`/step/3?scenario=${scenarioId}&level=${level}&mode=${mode}&useLocalLLM=${useLocalLLM}`);
  }, [router, scenarioId, level, mode, useLocalLLM]);
